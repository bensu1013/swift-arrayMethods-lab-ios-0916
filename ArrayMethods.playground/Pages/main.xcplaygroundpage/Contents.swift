 ...
 */
let numDaysOfTheWeek: [String] = daysOfTheWeek
...
if emptyArray.isEmpty{
    print("Sorry bud, it's empty")
} else {
    print("That's odd, when did that get in here?")
}
...
if !reverseEmptyArray.isEmpty {
    print("Yup, got a fish in here")
} else {
    print("Nope, nada, zilch")
}
 ...
 */
let arraySize: Int = daysOfTheWeek.count
print(arraySize)
...
if daysOfTheWeek.count > 5 {
    ...
    for (index, day) in daysOfTheWeek.enumerate() {
        if (day == "sunday") || (day == "saturday") {
            if indexArray.isEmpty {
                ...
                var temp: Int = index - indexArray.count
    ...
    for ind in indexArray {
        print(ind)
        daysOfTheWeek.removeAtIndex(ind)
    }
}
